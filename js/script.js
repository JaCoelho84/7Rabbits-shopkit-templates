$(document).ready(function() {
	...
	$('.products .product .img').each(function() {
		...
		$(this).imagesLoaded(function() {
			...
			description.css({ 'margin-top': -description_height / 2 });
			$(this).animate({ 'opacity': 1 }, 500);
		});
	});

	$('.products .product img').imagesLoaded(function() {
		products.masonry({ isAnimated: false });
	});
	...
	$('.cart-wrapper').hoverIntent({
		timeout: 500,
		over: function() {
			$('.cart-wrapper .popover').fadeIn('normal');
		},
		out: function() {
			$('.cart-wrapper .popover').fadeOut('normal');
		}
	});

	$('.cart-wrapper .popover').bind('clickoutside', function() {
		$('.cart-wrapper .popover').fadeOut('normal');
	});

	$('.cart-wrapper .popover a.close').click(function() {
		$('.cart-wrapper .popover').fadeOut('normal');
		return false;
	});
	...
	if (!navigator.userAgent.match(/msie/i)) {
		$(window).resize(function() {
			setTimeout(function() {
				//products.masonry('destroy').removeAttr('style'); /* fix chrome jump on resize */
				products.masonry({ isAnimated: true });
			}, 250);
		});
	}

	$('.categories > ul a[href="#"], .categories > a').click(function(e) {
		e.preventDefault();
	});
...
function slideshow_calc() {
	//var slideshow_height = $('.slideshow-wrapper').height();
	//var css_top = (slideshow_height/2)-slideshow_text_height/2;
	var slideshow_text_height = $('.slideshow-wrapper .text').height();
	$('.slideshow-wrapper .text').css({ 'margin-top': -slideshow_text_height / 2 });
